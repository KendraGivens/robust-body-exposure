...
class RobeReversibleEnv2(AssistiveEnv):
    def __init__(self):
        obs_robot_len = 28
        ...
        self.lower_before_release_reverse = True
        ...
        self.iteration = 0
        super(RobeReversibleEnv2, self).__init__(robot=None, human=Human(human_controllable_joint_indices, controllable=True), task='bedding_manipulation', obs_robot_len=obs_robot_len, obs_human_len=0, frame_skip=1, time_step=0.01, deformable=True)
        self.use_mesh = False
        self.reverse_action = True
        ...
        # replay_seed needs to be False when collecting new uncovered seeds and true when we want to resimulate those states
        # maybe seed val and replay seed need to be enviornmental variations (controlled by set_env_variations()) so this doesn't get mixed up
    ...
    def uncover_step(self, action):
        ...
        self.data_initial = p.getMeshData(self.blanket, -1, flags=p.MESH_DATA_SIMULATION_MESH, physicsClientId=self.id)
        ...
        dist, is_on_cloth = check_grasp_on_cloth(action, np.array(self.data_initial[1]), clipping_thres=.028)
        ...
        if not is_on_cloth:
            if self.collect_data:
                return self.obs, 0, False, {} # for data collect
            elif self.clip:
        ...
        if self.execute_action:
            ...
            grasp_loc = np.array(self.data_initial[1][self.anchor_idx[0]][0:2])
            ...
            self.sphere_ee.set_base_pos_orient(self.data_initial[1][self.anchor_idx[0]], np.array([0,0,0]))
            constraint_ids = []
            ...
            for i in self.anchor_idx[1:]:
                ...
                pos_diff = np.array(self.data_initial[1][i]) - np.array(self.data_initial[1][self.anchor_idx[0]])
            ...
            if self.take_images: self.capture_images("intermediate", self.iteration, self.min_threshold*100)

            return self.data_initial, self.data_intermediate

    def recover_step(self, action):
        action = scale_action(action) if not self.naive else scale_action(action, scale=[1, 1])

        #Perform second action
        if self.reverse_action:
            #Reverse the actions
            grasp_loc = action[4:6]
            release_loc = action[6:8]
            reverse_action =  [*grasp_loc, *release_loc]

            if self.new_anchors:
                # * calculate distance between the 2D grasp location and every point on the blanket, anchor points are the 4 points on the blanket closest to the 2D grasp location
                dist, is_on_cloth = check_grasp_on_cloth(reverse_action, np.array(self.data_intermediate[1]))
                # print("SIM", is_on_cloth, action[4:8])
                # * if no points on the blanket are within 2.8 cm of the grasp location, exit (if collecting data) or proceed without executing the action (in all other conditions)
                if not is_on_cloth:
                    if self.collect_data:
                        return self.obs, 0, False, {'cloth_initial':self.data_initial,'cloth_intermediate':np.array(self.data_intermediate[1]), 'action_r':reverse_action, 'action':action} # for data collect
                    elif self.clip:
                        self.execute_action = False
            ...
            if self.execute_action:
                ...
                # * update grasp_loc var with the location of the central anchor point on the cloth
                grasp_loc = np.array(self.data_intermediate[1][self.anchor_idx[0]][0:2])

                # * move sphere down to the anchor point on the blanket, create anchor point (central point first, then remaining points) and store constraint ids
                self.sphere_ee.set_base_pos_orient(self.data_intermediate[1][self.anchor_idx[0]], np.array([0,0,0]))
                constraint_ids = []
                constraint_ids.append(p.createSoftBodyAnchor(self.blanket, self.anchor_idx[0], self.sphere_ee.body, -1, [0, 0, 0]))
                for i in self.anchor_idx[1:]:
                    pos_diff = np.array(self.data_intermediate[1][i]) - np.array(self.data_intermediate[1][self.anchor_idx[0]])
                    constraint_ids.append(p.createSoftBodyAnchor(self.blanket, i, self.sphere_ee.body, -1, pos_diff))
                ...
                if self.lower_before_release_reverse:
                    current_pos = self.sphere_ee.get_base_pos_orient()[0]
                    delta_z = release_height_of_effector(np.array(current_pos), np.array(self.points_pos_limb_world), self.release_threshold)
                    #self.sphere_ee.set_base_pos_orient(current_pos + np.array([0, 0, delta_z]), np.array([0,0,0]))
                    final_z = current_pos[2] + delta_z
        ...
        # pickle.dump([self.data_initial, self.data_intermediate, data_final, action], open(f'not_working_test_data{time.time()}.pkl', 'wb'))
        ...
        if not self.collect_data:
            ...
            cloth_initial_subsample, cloth_intermediate_subsample, cloth_final_subsample = sub_sample_point_clouds_recover(self.data_initial[1], self.data_intermediate[1], data_final[1])
            # cloth_initial_subsample, cloth_final_subsample = sub_sample_point_clouds(self.data_initial[1], data_final[1])
            ...
            all_body_points = get_body_points_from_obs(human_pose, target_limb_code=self.target_limb_code, body_info=self.get_human_body_info())
            # TODO: make sure this is the recovering reward
            # recover_reward, covered_status = get_reward(action, all_body_points, cloth_initial_2D, cloth_final_2D)
            self.recover_reward, covered_status = get_recovering_reward(action, all_body_points, cloth_initial_2D, cloth_intermediate_2D, cloth_final_2D)
            #if self.take_images: self.capture_images(f"final_reward={recover_reward}", self.iteration, self.min_threshold*100)


            info = {
                "cloth_initial": self.data_initial,
        ...
        # return 0, 0, 1, {}
        return data_final, self.obs, self.recover_reward, done, info
    ...
    def reset(self):
        ...
        super(RobeReversibleEnv2, self).reset()
