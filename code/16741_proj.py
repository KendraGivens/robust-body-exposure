...
def uncover_cost_function(action, all_body_points, cloth_initial_raw, graph, uncover_model, recover_model, iter_data_dir, device, use_disp, use_3D):
    uncover_action = scale_action(action)
    ...
    if is_on_cloth:
        ...
        data['u'] = global_vec
        pred = uncover_model(data)['target'].detach().numpy()
    ...
    if use_3D:
        ...
        cost, covered_status = get_cost(action, all_body_points, cloth_initial, cloth_initial_2D, pred_2D)
    else:
        cost, covered_status = get_cost(action, all_body_points, cloth_initial, cloth_initial, pred)

    cloth_intermediate = np.hstack([pred, np.ones((pred.shape[0], 1))])
    ...
    graph = Runtime_Graph(
        ...
        edge_threshold=0.06,
        action_to_all=True,
        cloth_initial=cloth_intermediate,
    ...
    pass_through = {'pass_through': [all_body_points, cloth_initial, cloth_intermediate, graph, recover_model, device, use_disp, use_3D]}

    def recover_cost_function_helper(para):
        all_body_points, cloth_initial, cloth_intermediate, graph, recover_model, device, use_disp, use_3D = pass_through['pass_through']
        action = para_to_action(para)
        recov_cost, pred, covered_status, is_on_cloth = cost_function(action, all_body_points, cloth_initial, cloth_intermediate, graph, recover_model, device, use_disp, use_3D)
    ...
    recov_opt = gfo.RandomSearchOptimizer(search_space, initialize={"grid": 4, "random": 10, "vertices": 4, "warm_start": [x0]})
    # opt = gfo.ParallelTemperingOptimizer(search_space, initialize={"grid": 4, "random": 10, "vertices": 4, "warm_start": [x0]})
    t0 = time.time()
    recov_opt.search(
        recover_cost_function_helper,
        n_iter=20,
        verbosity=False)
    best_para = recov_opt.best_para
    best_recov_action = para_to_action(best_para)

    best_recov_cost, best_final_pred, best_covered_status, best_is_on_cloth = cost_function(best_recov_action, all_body_points, cloth_initial, cloth_intermediate, graph, recover_model, device, use_disp, use_3D)

    return [cost+best_recov_cost, [cost, pred, covered_status, is_on_cloth], [best_recov_action, best_recov_cost, best_final_pred, best_covered_status, best_is_on_cloth]]
    ...
    for eval_condition in eval_conditions:
        ...
        for f in filenames:
            if str(seed) in f.name:
                return f
    #%%
def optimizer(env_name, idx, uncover_model, recover_model, device, target_limb_code, iter_data_dir, graph_config, env_var, max_fevals):
    ...
    pass_through = {'pass_through': [all_body_points, cloth_initial_raw, graph, uncover_model, recover_model, iter_data_dir, device, use_disp, use_3D]}

    def uncov_cost_function_helper(para):
        all_body_points, cloth_initial_raw, graph, uncover_model, recover_model, iter_data_dir, device, use_disp, use_3D = pass_through['pass_through']
        action = para_to_action(para)
        cost, uncov_data, recov_data = uncover_cost_function(action, all_body_points, cloth_initial_raw, graph, uncover_model, recover_model, iter_data_dir, device, use_disp, use_3D)
        print(cost)
        return -cost
    ...
    best_total_cost, uncov_data, recov_data = uncover_cost_function(best_action, all_body_points, cloth_initial_raw, graph, uncover_model, recover_model, iter_data_dir, device, use_disp, use_3D)
    # best_reward = -best_cost
    print('BEST_TOTAL', best_total_cost)

    cloth_initial_sim, cloth_intermediate_sim, execute_uncover_action = env.uncover_step(best_action)
    cloth_final_sim, execute_recover_action = env.recover_step(recov_data[0]) # if not recovering, don't need to provide an action
    ...
    cma_info = [best_total_cost, uncov_data, recov_data]
    ...
    return seed, 0, uncover_reward, recover_reward, best_time, target_limb_code, False


def evaluate_dyn_model(env_name, target_limb_code, trials, uncover_model, recover_model, iter_data_dir, device, num_processes, graph_config, env_variations, max_fevals):
    ...
    for j in tqdm(range(math.ceil(trials/num_processes))):
        with multiprocessing.Pool(processes=num_processes) as pool:
            for i in range(num_processes):
                ...
                result = pool.apply_async(optimizer, args = (env_name, idx, uncover_model, recover_model, device, target_limb_code, iter_data_dir, graph_config, env_variations, max_fevals), callback=counter_callback)
...
if __name__ == '__main__':
    ...
    for i in range(len(loop_data)):
        ...
        checkpoint_uncover = '/home/kpputhuveetil/git/robe/robust-body-exposure/trained_models/FINAL_MODELS/standard_2D_not_subsampled_epochs=250_batch=100_workers=4_1687986938'
        checkpoint_recover = '/home/kpputhuveetil/git/robe/robust-body-exposure/trained_models/FINAL_MODELS/tl4_100_states_1k_1000_epochs=250_batch=50_workers=4_1700161836'
        ...
        data_dir = osp.join(checkpoint_uncover, f'cma_evaluations/TL_{target_limb_list}_{recover_string}_{test_string}_{args.num_rollouts}_states_RandomSearch_Opt')
        Path(data_dir).mkdir(parents=True, exist_ok=True)

        device = 'cpu'
        uncov_gnn_manager = GNN_Manager(device)
        uncov_gnn_manager.load_model_from_checkpoint(checkpoint_uncover)
        uncov_gnn_manager.model.to(torch.device('cpu'))
        uncov_gnn_manager.model.share_memory()
        uncov_gnn_manager.model.eval()

        recov_gnn_manager = GNN_Manager(device)
        recov_gnn_manager.load_model_from_checkpoint(checkpoint_recover)
        recov_gnn_manager.model.to(torch.device('cpu'))
        recov_gnn_manager.model.share_memory()
        recov_gnn_manager.model.eval()
        ...
        for iter in tqdm(range(iterations)):
            cma_reward, sim_reward, pred_sim_reward_error = evaluate_dyn_model(
                ...
                trials = trials,
                uncover_model = uncov_gnn_manager.model,
                recover_model = recov_gnn_manager.model,
